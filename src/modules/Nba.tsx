...
const NbaModule = () => {
  ...
  const fetchNbaMatches = async () => {
    ...
    setRadarResults([]);
    try {
      // Pedimos 'h2h' para obtener toda la programación garantizada
  ...
  const requestAiAnalysis = async () => {
    setAiLoading(true);
    const prompt = `
      Actúa como Senior NBA Handicapper. Analiza el juego: ${selectedMatch.home_team} vs ${selectedMatch.away_team}.
      DATOS: Spread esperado ${resultsH[0]?.expected || 'N/A'}, Total proyectado ${projTotal || 'N/A'}.
      EDGES DETECTADOS:
      ${resultsH.map(r => `* Spread ${r.team} [${r.line}] Edge: ${r.edge}`).join(', ')}
      ${resultsOU.map(r => `* Total ${r.type} [${r.value}] Edge: ${r.edge}`).join(', ')}
      NOTAS: ${analystNotes}
      Investiga: Lesiones de última hora (estrellas confirmadas), si es Back-to-Back (B2B) y el ritmo (Pace).
      Veredicto final: ¿En qué línea recomiendas invertir y por qué?
    `;

    try {
      const resp = await axios.post('/api/ai_analysis', { prompt });
      setAiResponse(resp.data.analysis);
    } catch (e) { setAiResponse("Error al contactar con OpenRouter."); }
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2 text-gray-500 text-[10px] font-bold uppercase tracking-widest mt-2">
             ...
             <span>Capital Shield NBA Intelligence (Colombia GMT-5)</span>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="glass-titanium rounded-[2rem] p-6 border-white/5">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-[10px] font-black uppercase text-orange-500 tracking-[0.2em] flex items-center gap-2"><Search size={14} /> 1. Jornada</h3>
              <button 
                onClick={runSmartRadar} 
                disabled={isScanning || filteredMatches.length === 0}
                className="px-3 py-1.5 bg-orange-500/10 text-orange-500 rounded-xl hover:bg-orange-500 hover:text-black transition-all text-[9px] font-black uppercase flex items-center gap-2 border border-orange-500/20"
              >
            ...
            <div className="space-y-2 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar text-white">
              {loading ? <p className="text-center py-20 text-[10px] animate-pulse">Sincronizando NBA...</p> : 
                ...
                }) : <div className="text-center py-20 opacity-30 uppercase text-[10px] tracking-widest">No hay juegos scheduleados</div>
              }
            </div>
          </div>
        </div>
                 ...
                 <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                    {handicaps.map((h, i) => (
                      <div key={`h-${i}`} className="bg-white/5 p-4 rounded-2xl border border-white/5 flex flex-col gap-2 animate-reveal">
                        <select onChange={(e) => { const n = [...handicaps]; n[i].team = e.target.value; setHandicaps(n); }} className="bg-transparent text-[10px] text-orange-500 font-black outline-none uppercase">
