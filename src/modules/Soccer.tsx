...
const SoccerModule = () => {
  ...
  // PROMPT 1: RADAR DE SELECCIÓN (EL QUE TE FALTABA)
  const copyRadarPrompt = () => {
    ...
    const prompt = `Actúa como Senior Quant Trader. Analiza esta cartelera de fútbol y selecciona los 3 eventos con más potencial de ineficiencia de cuotas:\n\n${list}\n\nResponde con un ranking 1-3 y una breve razón técnica por cada uno.`;
    navigator.clipboard.writeText(prompt);
    alert("Lista para Radar copiada. Pégala en tu IA para filtrar los mejores partidos.");
  };

  // PROMPT 2: VERDICTO MAESTRO
...
NOTAS ANALISTA: ${analystNotes || 'Ninguna'}

MISIÓN IA: Investiga lesiones, H2H, tiros a puerta y clima. ¿Cuál es la mejor línea para invertir hoy? Justifica si el valor matemático es real o trampa.`;
    navigator.clipboard.writeText(prompt);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 glass-titanium rounded-[2.5rem] p-8 border-white/5 shadow-2xl">
          ...
          <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
            {loading ? <p className="text-center py-20 text-[10px] animate-pulse uppercase tracking-widest text-emerald-500">Sincronizando...</p> : 
        ...
        <div className="lg:col-span-4 glass-titanium rounded-[2.5rem] p-8 border-t-emerald-500 border-t-4">
          <h3 className="text-[10px] font-black uppercase text-emerald-500 tracking-[0.2em] text-center mb-8"><Target size={16} className="inline mr-2"/> 2. Análisis</h3>
          
          <div className="grid grid-cols-2 gap-4 mb-6">
            <input type="number" placeholder="Home ELO" className="w-full bg-white/5 border border-white/10 p-5 rounded-[1.5rem] text-sm text-center outline-none focus:border-emerald-500" onChange={e => setEloH(e.target.value)} />
            <input type="number" placeholder="Away ELO" className="w-full bg-white/5 border border-white/10 p-5 rounded-[1.5rem] text-sm text-center outline-none focus:border-emerald-500" onChange={e => setEloA(e.target.value)} />
          </div>

          <div className="relative mb-6">
            <Hash className="absolute left-5 top-1/2 -translate-y-1/2 text-emerald-500" size={14} />
            <input type="number" placeholder="Total Proyectado" className="w-full bg-white/5 border border-white/10 p-5 pl-12 rounded-[1.5rem] text-xs outline-none focus:border-emerald-500" onChange={e => setProjTotal(e.target.value)} />
          </div>

          <textarea placeholder="Notas (lesiones, H2H, clima...)" className="w-full bg-white/5 border border-white/10 p-5 rounded-[1.5rem] text-xs h-20 mb-6 resize-none focus:border-emerald-500 outline-none text-white" onChange={e => setAnalystNotes(e.target.value)} />
          ...
          <div className="space-y-4 border-t border-white/5 pt-6">
            <div className="flex justify-between items-center px-2">
               ...
               <div className="flex gap-2">
                 <button onClick={() => setHandicaps([...handicaps, {team:'home', line:'', odds:''}])} className="p-2 bg-emerald-500/10 text-emerald-500 rounded-xl hover:bg-emerald-500 transition-all"><Plus size={14}/></button>
            ...
            <div className="space-y-3 max-h-[250px] overflow-y-auto">
              {handicaps.map((h, i) => (
                <div key={`h-${i}`} className="bg-white/5 p-3 rounded-xl border border-white/5 flex gap-2 animate-reveal">
                  ...
                  <input placeholder="Hnd" className="flex-1 bg-black/40 border border-white/10 p-1 rounded text-[10px] text-white text-center" onChange={e => {const n=[...handicaps]; n[i].line=e.target.value; setHandicaps(n);}} />
                  <input placeholder="Odds" className="w-12 bg-black/40 border border-white/10 p-1 rounded text-[10px] text-white text-center" onChange={e => {const n=[...handicaps]; n[i].odds=e.target.value; setHandicaps(n);}} />
              ...
              {ouLines.map((l, i) => (
                <div key={`ou-${i}`} className="bg-white/5 p-3 rounded-xl border border-white/5 flex gap-2 animate-reveal">
                  ...
                  <input placeholder="Tot" className="flex-1 bg-black/40 border border-white/10 p-1 rounded text-[10px] text-white text-center" onChange={e => {const n=[...ouLines]; n[i].value=e.target.value; setOuLines(n);}} />
                  <input placeholder="Odds" className="w-12 bg-black/40 border border-white/10 p-1 rounded text-[10px] text-white text-center" onChange={e => {const n=[...ouLines]; n[i].odds=e.target.value; setOuLines(n);}} />
                  <button onClick={() => setOuLines(ouLines.filter((_,idx)=>idx!==i))} className="text-red-900"><Trash2 size={12}/></button>
                </div>
              ))}
            </div>

            <button onClick={runFullAnalysis} disabled={isAnalyzing} className="w-full bg-emerald-600 p-6 rounded-[2rem] font-black text-[11px] uppercase tracking-[0.3em] shadow-xl hover:bg-emerald-500 transition-all active:scale-95">
                ...
                <div className="space-y-6">
                  {resultsH.map((r, i) => (
                    <div key={i} className="flex justify-between items-end border-b border-white/5 pb-4 last:border-0 text-white">
                      <div><div className={`text-[10px] font-black uppercase ${r.edge > 0.3 ? 'text-emerald-500' : 'text-gray-500'}`}>H: {r.team} [{r.line}]</div><div className="text-[9px] text-gray-600">Odds: {r.odds || 'N/A'}</div></div>
                      <div className={`text-3xl font-black italic tracking-tighter ${r.edge > 0.3 ? 'text-white' : 'text-gray-800'}`}>{r.edge}</div>
                    </div>
                  ))}
                  {resultsOU.map((r, i) => (
                    <div key={i} className="flex justify-between items-end border-b border-white/5 pb-4 last:border-0 text-white">
                      <div><div className={`text-[10px] font-black uppercase ${r.edge > 0.5 ? 'text-blue-500' : 'text-gray-500'}`}>OU: {r.type} [{r.value}]</div><div className="text-[9px] text-gray-600">Odds: {r.odds || 'N/A'}</div></div>
                      <div className={`text-3xl font-black italic tracking-tighter ${r.edge > 0.5 ? 'text-white' : 'text-gray-800'}`}>{r.edge}</div>
                    </div>
                  ))}
                </div>
              </div>
              <button onClick={copyMasterPrompt} className="w-full glass-titanium border-blue-500/40 p-8 rounded-[2.5rem] flex flex-col items-center gap-4 hover:border-blue-500/80 transition-all group active:scale-95 shadow-2xl">
