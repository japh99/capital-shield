...
const SoccerModule = () => {
  ...
  const fetchMatches = async (leagueId: string, dateStr: string) => {
    ...
    try {
      ...
      if (filtered.length === 0) {
        alert("No se encontraron partidos para esta fecha específica.");
      }
    } catch (e) {
      alert("Error de conexión. Verifica que el nombre de la variable en Vercel sea VITE_ODDS_KEYS");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleLeagueChange = (id: string) => {
    ...
    fetchMatches(id, selectedDate);
  };

  const handleDateChange = (date: string) => {
    setSelectedDate(date);
    if (selectedLeague) fetchMatches(selectedLeague, date);
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="glass-card p-6 border-white/5">
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
            {loading ? <div className="text-center py-10 animate-pulse text-[10px] text-emerald-500 uppercase">Sincronizando...</div> : 
              matches.map(m => (
                <button 
                  key={m.id} 
        ...
        <div className="glass-card p-6 border-t-4 border-emerald-600">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              <input type="number" placeholder="ELO Home" className="bg-black border border-white/10 p-4 rounded-xl text-xs" onChange={e => setEloH(e.target.value)} />
              <input type="number" placeholder="ELO Away" className="bg-black border border-white/10 p-4 rounded-xl text-xs" onChange={e => setEloA(e.target.value)} />
            </div>
        ...
        <div className="space-y-6">
          {results.length > 0 ? (
            <div className="space-y-4 animate-in zoom-in-95">
                    ...
                    <div key={i} className="flex justify-between items-center p-4 bg-black/40 rounded-xl border border-white/5 transition-all hover:border-white/20">
                      <div>
                        <div className={`text-[10px] font-bold uppercase ${r.team === 'home' ? 'text-emerald-500' : 'text-blue-500'}`}>
                          {r.team === 'home' ? 'Hándicap LOCAL' : 'Hándicap VISITANTE'} [{r.line}]
                        </div>
                        <div className="text-[9px] text-gray-500 mt-1">Cuota de Mercado: {r.odds}</div>
                      </div>
              ...
              <button className="w-full bg-blue-600/20 border border-blue-500/40 p-4 rounded-xl text-blue-400 font-black text-[10px] uppercase tracking-widest hover:bg-blue-600/40 transition-all">
            ...
            <div className="glass-card h-full flex flex-col items-center justify-center p-12 text-center border-dashed border-white/10 opacity-30">
              ...
              <p className="text-[10px] font-bold uppercase tracking-widest">Seleccione Evento y Configure Parámetros</p>
            </div>
          )}
        </div>
      </div>
