...
const SoccerModule = () => {
  ...
  const fetchMatches = async () => {
    ...
    } catch (e) { alert("Error de API Soccer."); }
  ...
  const requestAiAnalysis = async () => {
    setAiLoading(true);
    const prompt = `
      Actúa como Senior Quant Trader. Analiza: ${selectedMatch.home_team} vs ${selectedMatch.away_team}.
      DATOS: Margen goles ${resultsH[0]?.expected || 'N/A'}, Proyección Total ${projTotal || 'N/A'}.
      EDGES:
      ${resultsH.map(r => `* Handicap ${r.team} [${r.line}] Edge: ${r.edge}`).join(', ')}
      ${resultsOU.map(r => `* Total ${r.type} [${r.value}] Edge: ${r.edge}`).join(', ')}
      NOTAS: ${analystNotes}
      Misión: Investiga H2H, tiros a puerta recientes, bajas en portería/centrales y clima. 
      Veredicto: ¿Confirmas la inversión matemática o es un TRAP?
    `;
    try {
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="glass-titanium rounded-[2rem] p-6 border-white/5">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-[10px] font-black uppercase text-emerald-500 tracking-[0.2em] flex items-center gap-2"><Search size={14} /> 1. Mercado</h3>
              <button 
                onClick={runSmartRadar} 
                disabled={isScanning || filteredMatches.length === 0}
                className="px-3 py-1.5 bg-emerald-500/10 text-emerald-500 rounded-xl hover:bg-emerald-500 hover:text-black transition-all text-[9px] font-black uppercase flex items-center gap-2 border border-emerald-500/20"
              >
            ...
            <div className="space-y-3 mb-6 text-white">
              <select value={selectedLeague} onChange={(e) => setSelectedLeague(e.target.value)} className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-xs outline-none focus:ring-2 ring-emerald-500/20">
              ...
              <div className="grid grid-cols-12 items-center bg-white/5 border border-white/10 rounded-2xl overflow-hidden">
            ...
            <div className="space-y-2 max-h-[450px] overflow-y-auto pr-2 custom-scrollbar text-white">
              {loading ? <p className="text-center py-20 text-[10px] animate-pulse">Sincronizando...</p> : 
                filteredMatches.map(m => {
                  const priority = radarResults.find(r => r.teams.toLowerCase().includes(m.home_team.toLowerCase()));
                  return (
                    <button key={m.id} onClick={() => setSelectedMatch(m)} className={`w-full text-left p-5 rounded-2xl transition-all border relative overflow-hidden ${selectedMatch?.id === m.id ? 'bg-emerald-500/10 border-emerald-500/40 shadow-xl' : 'bg-white/5 border-transparent hover:bg-white/10'}`}>
        ...
        <div className="lg:col-span-4">
          <div className="glass-titanium rounded-[2rem] p-6 border-t-emerald-500/50 border-t-2">
            ...
            <div className="relative mb-6">
              ...
              <input type="number" placeholder="Proyección Goles Totales" className="w-full bg-white/5 border border-white/10 p-4 pl-10 rounded-2xl text-xs text-white outline-none focus:border-emerald-500" onChange={e => setProjTotal(e.target.value)} />
            </div>
            <textarea placeholder="Notas pro: Lesiones, Tiros a puerta, Clima..." className="w-full bg-white/5 border border-white/10 p-4 rounded-2xl text-xs text-white outline-none h-16 resize-none mb-6" onChange={e => setAnalystNotes(e.target.value)} />
               ...
               <div className="border-t border-white/5 pt-4">
                 <div className="flex justify-between items-center mb-3">
                   ...
                   <div className="flex gap-2">
                     <button onClick={() => setHandicaps([...handicaps, { team: 'home', line: '', odds: '' }])} className="p-1 bg-emerald-500/10 text-emerald-500 rounded-lg hover:bg-emerald-500"><Plus size={14}/></button>
                     <button onClick={() => setOuLines([...ouLines, { type: 'over', value: '', odds: '' }])} className="p-1 bg-blue-500/10 text-blue-500 rounded-lg hover:bg-blue-500"><BarChart3 size={14}/></button>
                   </div>
                 </div>
                 ...
                 <div className="space-y-2 max-h-[300px] overflow-y-auto">
                    {handicaps.map((h, i) => (
                      <div key={`h-${i}`} className="bg-white/5 p-3 rounded-xl border border-white/5 flex flex-col gap-2">
                        <select onChange={(e) => { const n = [...handicaps]; n[i].team = e.target.value; setHandicaps(n); }} className="bg-transparent text-[10px] text-emerald-500 font-bold outline-none uppercase">
                          <option value="home">Hándicap LOCAL</option><option value="away">Hándicap VISITANTE</option>
                        </select>
                        <div className="flex gap-2">
                          <input placeholder="Línea" className="flex-1 bg-black/40 border border-white/10 p-2 rounded text-xs text-white text-center font-bold" onChange={(e) => { const n = [...handicaps]; n[i].line = e.target.value; setHandicaps(n); }} />
                          <input placeholder="Cuota" className="w-16 bg-black/40 border border-white/10 p-2 rounded text-xs text-white text-center font-bold" onChange={(e) => { const n = [...handicaps]; n[i].odds = e.target.value; setHandicaps(n); }} />
                    ...
                    {ouLines.map((l, i) => (
                      <div key={`ou-${i}`} className="bg-white/5 p-3 rounded-xl border border-white/5 flex flex-col gap-2">
                        ...
                        <div className="flex gap-2">
                          <input placeholder="Total" className="flex-1 bg-black/40 border border-white/10 p-2 rounded text-xs text-white text-center font-bold" onChange={(e) => { const n = [...ouLines]; n[i].value = e.target.value; setOuLines(n); }} />
                          <input placeholder="Cuota" className="w-16 bg-black/40 border border-white/10 p-2 rounded text-xs text-white text-center font-bold" onChange={(e) => { const n = [...ouLines]; n[i].odds = e.target.value; setOuLines(n); }} />
            ...
            <div className="animate-reveal space-y-6">
               <div className="glass-titanium rounded-[2rem] p-8 border-l-4 border-emerald-500 relative overflow-hidden">
                  ...
                  <div className="space-y-8">
                    {resultsH.map((r, i) => (
                      <div key={`rh-${i}`} className="flex justify-between items-end border-b border-white/5 pb-4 last:border-0 text-white">
                        <div><div className={`text-[11px] font-black uppercase ${r.edge > 0.3 ? 'text-emerald-500' : 'text-gray-500'}`}>H: {r.team} [{r.line}]</div><div className="text-[9px] text-gray-600 uppercase">Cuota: {r.odds}</div></div>
                        <div className={`text-4xl font-black italic tracking-tighter ${r.edge > 0.3 ? 'text-white' : 'text-gray-800'}`}>{r.edge}</div>
                      </div>
                    ))}
                    {resultsOU.map((r, i) => (
                      <div key={`rou-${i}`} className="flex justify-between items-end border-b border-white/5 pb-4 last:border-0 text-white">
                        <div><div className={`text-[11px] font-black uppercase ${r.edge > 0.5 ? 'text-blue-500' : 'text-gray-500'}`}>OU: {r.type} [{r.value}]</div><div className="text-[9px] text-gray-600 uppercase">Cuota: {r.odds}</div></div>
